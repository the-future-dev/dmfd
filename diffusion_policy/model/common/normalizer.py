...
def _normalize(x, params, forward=True):
    ...
    # move scale & offset to wherever x lives, then cast x to scale.dtype
    device = x.device if isinstance(x, torch.Tensor) else scale.device
    scale = scale.to(device)
    offset = offset.to(device)
    x = x.to(device=device, dtype=scale.dtype)
    # now do the usual flatten / scale / reshape
    src_shape = x.shape
